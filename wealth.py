...
def fetch_finnhub_metrics(symbol):
    # 用 Finnhub 官方 API 抓基本面（美股穩定，不會像 yfinance 被 Yahoo 擋雲端 IP）。
    # 端點 /stock/metric?metric=all 會回傳 metric 字典，含 P/E、P/B、ROE、殖利率等。
    if not FINNHUB_KEY:
        return {}
    try:
        r = _requests.get(
        ...
        print(f"[finnhub] {symbol} 失敗: {e}")
    return {}

@st.cache_data(ttl=3600)
def fetch_fundamentals_and_news(symbol):
    # info：優先 Finnhub（穩），抓不到再退回 yfinance。news 仍走 yfinance。
    info = {}
    finn = fetch_finnhub_metrics(symbol)
    if finn:
        # 把 Finnhub 的欄位名對應成程式既有的 key，safe_metric 就能直接吃。
        info = {
            "trailingPE": finn.get("peTTM") or finn.get("peBasicExclExtraTTM"),
            "priceToBook": finn.get("pb") or finn.get("pbQuarterly"),
            "returnOnEquity": finn.get("roeTTM") or finn.get("roeRfy"),
            "dividendYield": finn.get("currentDividendYieldTTM") or finn.get("dividendYieldIndicatedAnnual"),
            "_source": "finnhub",
        }
    else:
        try:
            ...
            print(f"[fundamentals] {symbol} yfinance info 失敗: {e}")
    news = []
    try:
        news = yf.Ticker(symbol).news or []
    except Exception as e:
        print(f"[fundamentals] {symbol} news 失敗: {e}")
    return info, news
    ...
    for k in keys:
        ...
        if val is not None and val != "Infinity" and val != "":
            try:
                v = float(val)
                if is_percent:
                    # Finnhub 的 ROE/殖利率多半已是百分比(如 15.3)；yfinance 多為小數(0.153)。
...
# =============================================================
    ...
    if price and hist is not None:
        ...
        with tab_fund:
            ...
            fc3.metric("ROE", safe_metric(info, ['returnOnEquity', 'returnOnAssets'], is_percent=True))
            fc4.metric("殖利率", safe_metric(info, ['dividendYield', 'trailingAnnualDividendYield', 'yield'], is_percent=True))
            if info.get("_source") == "finnhub":
                st.caption("資料來源：Finnhub（美股基本面）")
            no_data = not info or all(safe_metric(info, [k]) == "無資料"
                                      for k in ['trailingPE', 'priceToBook', 'returnOnEquity', 'dividendYield'])
            if no_data:
                with st.expander("⚠️ 基本面暫無資料（點開看診斷）"):
                    if not FINNHUB_KEY:
                        st.warning("尚未設定 FINNHUB_API_KEY 環境變數。設定後美股基本面就會穩定顯示。")
                    else:
                        st.caption("已設定 Finnhub key，但此標的可能無此欄位（台股 Finnhub 免費版支援有限）。")
                    st.write(f"目前抓到的欄位：{ {k: info.get(k) for k in ['trailingPE','priceToBook','returnOnEquity','dividendYield']} }")
