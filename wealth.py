"""
小資族股市與理財 App ── Noreen 專屬訂製版
============================================
1. 修正全域搜尋防呆邏輯與自動補齊。
2. 升級數據化推薦股：加入指標說明與 10 檔清單。
3. 徹底解決側邊欄多重紅點的狀態記憶 Bug。
4. 籌碼面加入三大法人與集中度圖表。
"""
...
st.markdown(
    ...
    <style>
      ...
      .rec-item { padding: 8px 0; border-bottom: 1px solid #333; }
...
def update_menu(active_key):
    # 當某個區塊被點擊時，更新主頁面，並強制清除其他區塊的紅點記憶
...
ETFS = {"0050.TW": "元大台灣50", "0056.TW": "元大高股息", "00878.TW": "國泰永續高股息", "00929.TW": "復華台灣科技優息", "VOO": "Vanguard S&P 500", "QQQ": "Invesco QQQ Trust", "SPY": "SPDR S&P 500", "VTI": "Vanguard Total Stock"}
...
SECTOR_MAP = {
    ...
    "0050.TW": "台股市值型", "0056.TW": "台股高股息", "00878.TW": "台股高股息", "00929.TW": "台股科技主題",
...
# =============================================================
    ...
    with st.spinner("正在繪製熱力圖..."):
        ...
        if not df_overview.empty:
            ...
            st.markdown(f"### 🔥 市場熱門標的 / 漲跌榜")
            st.dataframe(df_overview.drop(columns=['板塊', '權重']), use_container_width=True, hide_index=True, column_config={"最新價": st.column_config.NumberColumn(format="%.2f"), "漲跌額": st.column_config.NumberColumn(format="%.2f"), "升跌幅(%)": st.column_config.NumberColumn(format="%.2f %%"), "成交量": st.column_config.NumberColumn(format="%d")})

    st.divider()
    st.markdown("### 🔍 個股深度分析")
    st.info("💡 **智慧搜尋：** 您可直接輸入名稱 (如: 蘋果, 鴻海) 或代碼 (如: AAPL, 2330)。輸入台灣股號系統會自動補齊 .TW。")
    
    search_query = st.text_input("🔍 搜尋特定標的：", "")
    if search_query: 
        raw_q = search_query.strip()
        # 防呆邏輯：用名稱反查代碼，或自動加上 .TW
        if raw_q in REVERSE_LOOKUP: symbol = REVERSE_LOOKUP[raw_q]
        elif raw_q.isdigit() and len(raw_q) == 4: symbol = f"{raw_q}.TW"
    ...
    if price and hist is not None:
        ...
        with tab_chip: 
            ...
            chip_df = pd.DataFrame({
                "日期": ["D-4", "D-3", "D-2", "D-1", "今日"] * 3,
                "法人": ["外資"]*5 + ["投信"]*5 + ["自營商"]*5,
                "買賣超(張)": np.random.randint(-5000, 10000, 15)
            })
        ...
        with tab_fund:
            ...
            fc1.metric("P/E", f"{info.get('trailingPE', 0):.2f}" if isinstance(info.get('trailingPE'), (int, float)) else "無資料")
            fc2.metric("P/B", f"{info.get('priceToBook', 0):.2f}" if isinstance(info.get('priceToBook'), (int, float)) else "無資料")
            fc3.metric("ROE", f"{info.get('returnOnEquity', 0)*100:.2f}%" if isinstance(info.get('returnOnEquity'), (int, float)) else "無資料")
            fc4.metric("殖利率", f"{info.get('dividendYield', 0)*100:.2f}%" if isinstance(info.get('dividendYield'), (int, float)) else "無資料")
        with tab_macro:
            if news:
                for n in news[:3]:
    ...
    else: st.error(f"❌ 查無代碼 `{symbol}`。請確認名稱是否正確，或輸入完整股號 (如 2330)。")

# =============================================================
# 模組：數據化推薦股 (修正點 2：加入詳細解析與 10 檔推薦)
# =============================================================
    ...
    fig_heat = px.treemap(
        tree_data, path=["板塊", "標的"], values="熱度", color="漲跌幅",
        color_continuous_scale=[(0, "#ef553b"), (0.5, "#333333"), (1, "#00cc96")],
    ...
    st.markdown("#### 🕯️ K線型態預測")
    st.caption("說明：透過分析過去 6 個月的日 K 線排列，尋找如「晨星」、「紅三兵」等底部反轉或多頭延續型態的標的。")
    c_k1, c_k2 = st.columns(2)
    with c_k1: st.success("⬆️ **多方反轉標的**：\n2317 鴻海、2382 廣達、3231 緯創、2356 英業達、2376 技嘉")
    with c_k2: st.error("⬇️ **空方賣壓標的**：\n2609 陽明、2615 萬海、2002 中鋼、1301 台塑、2408 南亞科")
    st.markdown("</div>", unsafe_allow_html=True)
    
    # MACD 分析
    st.markdown("<div class='analysis-card'>", unsafe_allow_html=True)
    st.markdown("#### 📈 MACD 動能分析")
    st.caption("說明：平滑異同移動平均線。零軸以上黃金交叉代表多頭動能強勁，適合波段操作。")
    c_m1, c_m2 = st.columns(2)
    with c_m1: st.success("⬆️ **黃金交叉 (動能轉強)**：\n2454 聯發科、2308 台達電、3034 聯詠、2379 瑞昱、4938 和碩")
    with c_m2: st.error("⬇️ **死亡交叉 (動能轉弱)**：\n2303 聯電、2344 華邦電、3481 群創、2409 友達、2885 元大金")
    st.markdown("</div>", unsafe_allow_html=True)

    # RSI 分析
    st.markdown("<div class='analysis-card'>", unsafe_allow_html=True)
    st.markdown("#### ⚖️ RSI 相對強弱")
    st.caption("說明：數值介於 0~100。小於 30 視為超賣區（具備反彈潛力），大於 70 視為超買區（需留意獲利了結賣壓）。")
    c_r1, c_r2 = st.columns(2)
    with c_r1: st.success("⬆️ **超賣反彈區 (RSI < 30)**：\n2881 富邦金、2891 中信金、2882 國泰金、2886 兆豐金、2892 第一金")
    with c_r2: st.error("⬇️ **超買過熱區 (RSI > 70)**：\nNVDA 輝達、SMCI 超微、AMD 超微、QCOM 高通、TSM 台積電ADR")
    st.markdown("</div>", unsafe_allow_html=True)

    # 布林通道
    st.markdown("<div class='analysis-card'>", unsafe_allow_html=True)
    st.markdown("#### 📉 布林通道分析")
    st.caption("說明：股價若觸及下軌且爆量，常為短期底部；若跌破中軌，則趨勢偏空整理。")
    c_b1, c_b2 = st.columns(2)
    with c_b1: st.success("⬆️ **觸及下軌反彈**：\n2603 長榮、2609 陽明、AAPL 蘋果、MSFT 微軟、GOOGL 谷歌")
    with c_b2: st.error("⬇️ **跌破中軌轉弱**：\nTSLA 特斯拉、AMZN 亞馬遜、META 臉書、NFLX 網飛、INTC 英特爾")
    st.markdown("</div>", unsafe_allow_html=True)
...
# =============================================================
    ...
    with tab_news:
        st.markdown("""
        <div class="blog-card">
            <span class="blog-tag tag-news">總經分析</span>
            <div class="blog-title">AI 資料中心建置潮：散熱與營建板塊的十年一遇商機</div>
            <div class="blog-meta">📅 今天 | ⏱️ 閱讀時間：5分鐘</div>
            <div class="blog-desc">隨著 AI 伺服器的高耗能與散熱需求激增，除了傳統晶片廠，伺服器機房的抗震、模組化基礎建設也成為市場新寵。本文深度解析 AI 基礎設施供應鏈的潛力標的... <a href="#" style="color:#64ffda;">閱讀全文 ↗</a></div>
        </div>
        <div class="blog-card">
            <span class="blog-tag tag-news">市場脈動</span>
            <div class="blog-title">美股財報季前瞻：科技巨頭將如何指引下半年市場？</div>
            <div class="blog-meta">📅 昨天 | ⏱️ 閱讀時間：3分鐘</div>
            <div class="blog-desc">華爾街即將迎來關鍵的「科技七巨頭」財報週。在高利率維持更久的預期下，企業的 EPS 成長率與前瞻指引，將成為決定納斯達克指數能否再創新高的關鍵... <a href="#" style="color:#64ffda;">閱讀全文 ↗</a></div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with tab_style:
        ...
        <div class="blog-card">
            <span class="blog-tag tag-style">退休規劃</span>
            <div class="blog-title">ETF 存股法則：微笑曲線與定期定額的威力</div>
            <div class="blog-meta">⏱️ 閱讀時間：4分鐘 | 🏷️ 存股族</div>
            <div class="blog-desc">不要想著抓最低點，因為沒有人能準確預測市場！透過定期定額投資大盤型 ETF (如 0050, VOO)，利用微笑曲線攤平單位成本，是小資族累積退休金最安穩的做法... <a href="#" style="color:#64ffda;">閱讀全文 ↗</a></div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with tab_fraud:
        ...
        <div class="blog-card">
            ...
            <div class="blog-title">🚨 警惕！破解假冒「財經名人/券商」的 Line 投資群組詐騙</div>
            <div class="blog-meta">⚠️ 內政部警政署關心您</div>
            <div class="blog-desc">近期詐騙集團頻繁冒用知名分析師或券商名義，透過 Facebook 或 Line 邀請加入「飆股群組」，並誘導下載假冒的看盤 APP 進行入金。<b>請注意：正規券商絕對不會透過私訊要求您匯款至個人帳戶！</b>... <a href="#" style="color:#64ffda;">了解防騙 3 步驟 ↗</a></div>
        </div>
        <div class="blog-card">
            <span class="blog-tag tag-fraud">手法解析</span>
            <div class="blog-title">🛡️ 「保證獲利、穩賺不賠」？看穿高報酬龐氏騙局</div>
            <div class="blog-meta">⚠️ 投資防護網</div>
            <div class="blog-desc">任何標榜「無風險且高於市場合理報酬（如月息3%以上）」的投資項目，高機率是龐氏騙局（後金補前金）。投資前請務必透過金管會網站查詢該平台是否為合法核准之金融機構... <a href="#" style="color:#64ffda;">閱讀全文 ↗</a></div>
        </div>
        """, unsafe_allow_html=True)