...
if page == "🕒 進度 (Progress)":
    ...
    # 修復「幽靈框框」：原本用裸 <div> 包 Streamlit 元件，元件不會被包進去，
    # 導致出現一個空殼卡片。改用 st.container(border=True) 才能真正包住裡面的輸入元件。
...
# =============================================================
    ...
    # 防呆：抓不到指數資料時（網路／yfinance 失敗）顯示提示，避免整頁崩潰。
    cols = [i1, i2, i3]
    ...
    with st.spinner("正在繪製熱力圖..."):
        ...
        if not df_overview.empty:
            ...
            fig_heat.update_layout(margin=dict(t=30, b=0, l=0, r=0), height=450, paper_bgcolor="rgba(0,0,0,0)", font=dict(color='#808080'))
...
# =============================================================
    ...
    fig_heat.update_layout(height=400, margin=dict(t=0, b=0, l=0, r=0), paper_bgcolor="rgba(0,0,0,0)", font=dict(color='#808080'))
    st.plotly_chart(fig_heat, use_container_width=True)
