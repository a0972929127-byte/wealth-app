...
def fetch_fundamentals_and_news(symbol):
    # 分開抓 info 與 news：新版 yfinance 的 .info 容易整包失敗，
    # 一旦失敗就會讓基本面全空。拆開各自 try，互不影響。
    info = {}
    news = []
    try:
        ticker = yf.Ticker(symbol)
        # get_info() 比 .info 在新版更穩；取不到再退回 .info
        try:
            info = ticker.get_info() or {}
        except Exception:
            info = ticker.info or {}
    except Exception as e:
        print(f"[fundamentals] {symbol} info 失敗: {e}")
    try:
    ...
    for k in keys:
        ...
        if val is not None and val != "Infinity" and val != "":
            try:
                v = float(val)
                if is_percent:
                    # yfinance 的殖利率/ROE 有時回小數(0.043)、有時回百分比(4.3)。
                    # 若數值很小(<1)視為小數需 ×100；否則視為已是百分比。
...
# =============================================================
    ...
    if price and hist is not None:
        ...
        with tab_fund:
            ...
            # 診斷用：若四格都無資料，代表 yfinance 這次沒回傳基本面，展開可看實際抓到的欄位
            if not info or all(k not in info for k in ['trailingPE', 'priceToBook', 'returnOnEquity', 'dividendYield']):
                with st.expander("⚠️ 基本面暫無資料（點開看診斷）"):
                    st.caption("yfinance 這次回傳的資料來源可能暫時失效，或該標的無此欄位。")
                    st.write(f"抓到的欄位數量：{len(info) if info else 0}")
                    if info:
                        st.json({k: info.get(k) for k in
                                 ['trailingPE', 'forwardPE', 'priceToBook', 'returnOnEquity',
                                  'dividendYield', 'trailingAnnualDividendYield', 'marketCap', 'shortName']})
