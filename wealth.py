...
if page == "🕒 進度 (Progress)":
    ...
    with c1:
        ...
        fig.update_layout(showlegend=False, margin=dict(t=0, b=0, l=0, r=0), height=200, paper_bgcolor="rgba(0,0,0,0)", font=dict(color='inherit'))
...
# =============================================================
    ...
    with st.spinner("正在繪製熱力圖..."):
        ...
        if not df_overview.empty:
            ...
            df_overview['權重'] = 1 
            fig_heat = px.treemap(df_overview, path=["板塊", "代碼"], values="權重", color="升跌幅(%)", color_continuous_scale=[(0, "#ef553b"), (0.5, "rgba(128,128,128,0.2)"), (1, "#00cc96")], color_continuous_midpoint=0)
            fig_heat.update_traces(texttemplate="<b>%{label}</b><br>%{color:.2f}%", textposition="middle center")
            fig_heat.update_layout(margin=dict(t=30, b=0, l=0, r=0), height=450, paper_bgcolor="rgba(0,0,0,0)", font=dict(color='inherit'))
    ...
    if price and hist is not None:
        ...
        with tab_tech:
            ...
            fig_k.update_layout(height=450, margin=dict(l=0,r=0,t=30,b=0), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font=dict(color='inherit'), xaxis_rangeslider_visible=False)
        ...
        with tab_chip: 
            ...
            fig_chip.update_layout(height=300, margin=dict(l=0,r=0,t=10,b=0), paper_bgcolor="rgba(0,0,0,0)", font=dict(color='inherit'))
            ...
            with c_gauge:
                ...
                fig_gauge.update_layout(height=200, margin=dict(l=10,r=10,t=30,b=10), paper_bgcolor="rgba(0,0,0,0)", font=dict(color='inherit'))
        ...
        with tab_macro:
            if news:
                for n in news[:3]:
                    ...
                    st.markdown(f"""<div class="news-card"><h4 style="margin:0; padding-bottom:5px;">{t}</h4><span style="font-size:0.8rem; opacity:0.7;">{p}</span><br><a href="{l}" target="_blank" style="color:#00cc96; text-decoration:none;">閱讀全文 ↗</a></div>""", unsafe_allow_html=True)
...
# =============================================================
    ...
    fig_heat = px.treemap(tree_data, path=["板塊", "標的"], values="熱度", color="漲跌幅", color_continuous_scale=[(0, "#ef553b"), (0.5, "rgba(128,128,128,0.2)"), (1, "#00cc96")], color_continuous_midpoint=0)
    fig_heat.update_layout(height=400, margin=dict(t=0, b=0, l=0, r=0), paper_bgcolor="rgba(0,0,0,0)", font=dict(color='inherit'))
    st.plotly_chart(fig_heat, use_container_width=True)
...
# =============================================================
    ...
    with tab_news:
        ...
        <div class="blog-card">
            ...
            <div class="blog-desc">隨著 AI 伺服器的高耗能與散熱需求激增，除了傳統晶片廠，資料中心機房的基礎建設也迎來爆發。為了應對伺服器重量與潛在風險，機櫃的 7 級抗震規範、模組化管線支撐系統正成為供應鏈中不可忽視的「硬底子」商機... <a href="#" style="color:#00cc96;">閱讀全文 ↗</a></div>
        </div>
        <div class="blog-card">
            ...
            <div class="blog-desc">當 AI 模型從雲端走向落地應用，智慧工廠與自動化生產線的數據傳輸成為關鍵。具備高穩定性與抗惡劣環境的工業級網通設備需求正迎來強勁成長... <a href="#" style="color:#00cc96;">閱讀全文 ↗</a></div>
        </div>
        <div class="blog-card">
            <span class="blog-tag tag-news">市場脈動</span>
            <div class="blog-title">美股財報季前瞻：科技巨頭將如何指引下半年市場？</div>
            <div class="blog-meta">📅 本週精選 | ⏱️ 閱讀時間：3分鐘</div>
            <div class="blog-desc">華爾街即將迎來關鍵的「科技七巨頭」財報週。在高利率維持更久的預期下，企業的 EPS 成長率與前瞻指引，將成為決定納斯達克指數能否再創新高的關鍵要素... <a href="#" style="color:#00cc96;">閱讀全文 ↗</a></div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with tab_style:
        ...
        <div class="blog-card">
            ...
            <div class="blog-desc">投資不只是與市場博弈，更是認識自己的過程！了解自己的設計能幫助建立更穩定的心態。例如，擁有「5/1 異端探究者」特質的人，往往擅長深入研究底層邏輯，非常適合挖掘基本面被低估的好公司... <a href="#" style="color:#c48a44;">閱讀全文 ↗</a></div>
        </div>
        <div class="blog-card">
            ...
            <div class="blog-desc">存錢不代表要過得刻苦。把錢集中花在真正喜歡且耐用的事物上，能幫你更快累積投資本金... <a href="#" style="color:#5058c9;">閱讀全文 ↗</a></div>
        </div>
        <div class="blog-card">
            ...
            <div class="blog-desc">股市中兩大最經典的門派！一文看懂如何根據自己的風險承受度配置資產... <a href="#" style="color:#5058c9;">閱讀全文 ↗</a></div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with tab_fraud:
        ...
        <div class="blog-card">
            ...
            <div class="blog-desc">詐騙集團常透過交友軟體建立感情，接著誘導受害者將資金轉入「特製的假虛擬貨幣交易所」。一開始會讓你有小額獲利並成功提現，當你投入大筆資金後，平台就會以「需繳納保證金」或「帳戶凍結」為由拒絕出金... <a href="#" style="color:#d64a33;">閱讀全文 ↗</a></div>
        </div>
        <div class="blog-card">
            ...
            <div class="blog-desc">近期詐騙集團頻繁冒用知名分析師或券商名義，透過 Facebook 或 Line 邀請加入「飆股群組」，並誘導下載假冒的看盤 APP 進行入金。<b>請注意：正規券商絕對不會透過私訊要求您匯款至個人帳戶！</b>... <a href="#" style="color:#d64a33;">了解防騙 3 步驟 ↗</a></div>
        </div>
        """, unsafe_allow_html=True)
